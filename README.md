...
### Railway 배포
- 🚂 **Dockerfile** 추가: Node.js + Python 가상환경 기반 컨테이너 빌드
- 📦 **PEP 668 대응**: Python venv 사용으로 배포 환경 호환성 확보
- 📖 [RAILWAY_DEPLOY.md](./RAILWAY_DEPLOY.md), [DEPLOYMENT_OPTIONS.md](./DEPLOYMENT_OPTIONS.md) 배포 가이드 추가

### 보고서 생성 안정성 개선
- 📄 **CSV 인코딩 폴백**: UTF-8, CP949, EUC-KR, Latin-1 등 다중 인코딩 자동 시도
- 📊 **컬럼명 유연 처리**: 대소문자 무관 (sales/Sales), 공백 자동 제거
- 🔧 **에러 메시지 개선**: Python 스크립트 오류 시 상세 메시지 클라이언트 전달
- 📁 **작업 디렉터리 고정**: spawn 시 `cwd` 설정으로 경로 이슈 해결
